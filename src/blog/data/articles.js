const articles = [{
    "id" : 2,
    "title": "It's Been A While: React, AWS Hosting, and Dockerfiles",
    "category": "general",
    ...
    "content": [
        ...
        "For moving quicker on the site, I've dug into a course from Tyler McGinnis on React Fundamentals. It's a subscription based service that isn't too bad. The fundamentals course is thorough. Ties everything together and covers some essentials but in the parts you already know (which for the average dev may be the first half if not a little more), it can be a little painful and the explanations are geared towards younger developers. I get the sense it was built with a specific timeline in mind and then things were adjusted after the fact. Despite the reality this just happens to be the way projects turn out and is understandable, it's slightly jarring.",
        "He has other things like React Native, React Router and a few others. It's worth checking out although I should mention it has a $20/month subscription fee. Overall I'd say I'm happy with it. For the gaps that it did fill in for me, it was well worth the investment. There are references throughout the courses to the free React Fundamentals course..it's not free, sorry you were too late...so was I :(",
        "With this update I'm going to be adding mobile support. Using flexbox is an ideal approach towards laying out the user interface in an easy to reason about manner. I like SCSS for the more programmatic syntax which feels more natural while working within javascript.",
        ...
        "So a mobile ready site, category tags, and then some information in the 'About Me' section. Those are the pieces that I know need some love at the moment. Having a filter that would easily organize on category would be nice..but first it'll need some categories! Also we'll need to deal with the 'media-sharing' links which will require some state. An inexpensive store where I can throw a simple table would be just right for that.",
        "Got the bill for AWS..$20 bucks for this site, ugh... Not much but I know I can drive it down to half of that using EC2 only. Currently I enjoy the convenience of Dockerfiles on Elastic Beanstalk a little too much...too much to let it go just yet. We drop the instance size down first and iterate from there.",
        "The beauty of using the Dockerfile is we can spin this application up locally essentially letting us deploy anywhere. That's a big deal and is worth the cost for the consistency between environments it provides."
    ],
